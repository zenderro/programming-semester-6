...
1. $T=A^{-1}F=[A^{-1}f_1,\dots,A^{-1}f_m]$ - вектора коэфф. интерполирующих ф-ий $u$, построенных по $f_k$.
2. $Г=T(B^t)^{-1} = (B^{-1}T^t)^t = [B^{-1}t_1,\dots,B^{-1}t_n]^t$ - вектора коэфф. интерполирующих ф-ий $v$, построенных по $t_l$.
...
Ссылка на задание: [https://classroom.github.com/a/0yeb7CAx](https://classroom.github.com/a/0yeb7CAx). Заготовка будет в созданном репозитории.